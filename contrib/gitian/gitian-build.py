...
gsigs = 'https://git.lolnero.com/qvqc/gitian.sigs.git'
gbrepo = 'https://git.lolnero.com/qvqc/gitian-builder.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('lolnero'):
        subprocess.check_call(['git', 'clone', args.url, 'lolnero'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/lolnero/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'lolnero='+args.commit, '--url', 'lolnero='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/lolnero-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/lolnero/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/lolnero/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://git.lolnero.com/lolnero/lolnero', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/lolnero', exist_ok=True)
    os.chdir('builder/inputs/lolnero')
    if args.pull:
